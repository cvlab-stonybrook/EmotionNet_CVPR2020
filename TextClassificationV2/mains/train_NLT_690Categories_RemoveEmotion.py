...
from TextClassificationV2.data_loader.Emotion690 import read_690_complete_mtrain_mtest_wo_emotion, convert_list2multihot
...
def ifContainCoreWords(tag2idx):
    emotion690_vocabulary = loadpickle(os.path.join(project_root, '/AdobeStockSelection/RetrieveSelected778/data_v6_690_xmas/Emotion_vocabulary.pkl'))
...
def create_data_args(data_splits, num_classes=690):
    args_data = Namespace()
    args_data.vocab = sorted(list(set([w for x_split in data_splits for sent in x_split for w in sent[0]])))
    args_data.classes = [x for x in range(num_classes)] # FIXME: do this in a better way!
...
def main():
    ...
    parser.add_argument("--lr_schedule", '-lrs', default='10,15', type=str, help="the schedule of learning rate")
    ...
    parser.add_argument('--w2v_type', default='googlenews', help='selftrained | googlenews')
    ...
    train_set, val_set = read_690_complete_mtrain_mtest_wo_emotion(subset_N=None)

    args_data = create_data_args([train_set, val_set], 690)
    ...
    save_checkpoint(save_items, is_best=False, filename='../ckpts/TextCNN_{}_690Removed_NLTDAN_1M_Static.pth.tar'.format(args_hyper.w2v_type))
...
def train(train_set, val_set, args_data, args_model, args_hyper):
    ...
    wordvec_file = os.path.join(project_root, 'LanguageData/word2vec_dicts/' \
    ...
    model = TextCNN(args_model, init_wv=wv_matrix).cuda(args_hyper.gpu)
    ...
    for e in range(args_hyper.epoch):
        ...
        for i in tqdm.tqdm(range(0, len(train_set), args_hyper.batch_size)):
            ...
            batch_y = [convert_list2multihot(c[1], args_model.num_classes) for c in train_set[i:i + batch_range]]


            torch_x = Variable(torch.LongTensor(batch_x)).cuda(args_hyper.gpu)
            torch_y = Variable(torch.FloatTensor(batch_y)).cuda(args_hyper.gpu)
        ...
        dev_top1, dev_hits, dev_loss = test(val_set, model, args_data, args_model, args_hyper)
...
def test(val_set, model, args_data, args_model, args_hyper):
    model.eval()
    ...
    y = [convert_list2multihot(c[1], args_model.num_classes) for c in val_set]

    x_torch = Variable(torch.LongTensor(x)).cuda(args_hyper.gpu)
    y_torch = Variable(torch.FloatTensor(y)).cuda(args_hyper.gpu)
