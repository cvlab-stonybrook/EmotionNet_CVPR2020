...
def main():
    text_ckpt = torch.load('/home/zwei/Dev/AttributeNet3/TextClassificationV2/ckpts/TextCNN_googlenews_690Removed_NLTDAN_1M_Static.pth.tar', map_location=lambda storage, loc:storage)
    ...
    idx2emotion = loadpickle('/home/zwei/Dev/AttributeNet3/AdobeStockSelection/EmotionNetFinal/Emotion_vocabulary.pkl')['idx2key']
    ...
    for data_idx, s_data in enumerate(dataset):
        ...
        if image_cid in image_url_dict:
            ...
            print(', '.join('{}({:.2f})'.format(idx2emotion[i], predicts[i]) for i in arg_max_predict))
