...
if __name__ == '__main__':
    ...
    import argparse
    import glob
    input_file = '/home/zwei/Dev/PastProjects/AttributeNet3/Dataset_release/StockEmotion/image_download_list/dataset_image_urls_240.pkl'
    ...
    parser.add_argument('--output_directory', '-t', default='StockEmotion', type=str, help='output filename')
    ...
    for s_idx, (s_annotation) in enumerate(tqdm.tqdm(selected_annotations)):

            # s_cid = s_annotation['cid']

            s_target_file = s_annotation[0]
            s_target_path = os.path.join(args.output_directory, s_target_file)
            s_target_subdir = os.path.dirname(s_target_path)
            if not os.path.exists(s_target_subdir):
                os.makedirs(s_target_subdir)


            s_url =s_annotation[1]
            ...
            s_tuple = (s_target_file, s_url, s_target_path)
