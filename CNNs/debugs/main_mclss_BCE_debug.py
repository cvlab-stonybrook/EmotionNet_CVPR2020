...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Pytorch Image CNN training from Configure Files - Debug version")
    ...
    # Data loading code
    val_dataset = get_instance(custom_datasets, '{0}_test'.format(args.dataset['name']), args, **args.dataset['args'])
    ...
    if args.evaluate:
        ...
        train_dataset = get_instance(custom_datasets, '{0}_test'.format(args.dataset['name']), args, **args.dataset['args'])
...
def validate(val_loader, model, criterion, args, print_func):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            # prec1 = CNN_utils.accuracy(output, target, topk=(1,))
            losses.update(loss.item(), input.size(0))
