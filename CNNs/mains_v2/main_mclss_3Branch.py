...
def train(train_loader, model, text_model, criterion, optimizer, epoch, args, print_func):
    ...
    for i, (input, target, text) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            text_cls = F.softmax(text_output[0], dim=1)
        # text_class_values, text_class_ids = torch.max(text_output[0], dim=1)
        # text_class_ids[text_class_values<0.5] = 4
        # compute output
        output_cls, output_proj, output_cls_basic = model(input)
        loss_cls = mCEL(output_cls, target)
        ...
        loss = loss_cls + args.alpha* loss_ebd + args.beta * loss_cls_aux
...
def validate(val_loader, model, criterion, args, print_func):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            log_softmax_output = F.log_softmax(output[0], dim=1)

            loss = - torch.sum(log_softmax_output * target)/ output[0].shape[0]
            # measure accuracy and record loss
            prec1 = CNN_utils.accuracy_multihots(output[0], target, topk=(1, 3))
