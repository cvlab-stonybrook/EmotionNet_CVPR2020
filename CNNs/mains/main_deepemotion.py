...
def main():
    ...
    if args.resume is None:
        if args.save_directory is None:
            save_directory = get_dir(os.path.join(project_root, 'ckpts', '{:s}'.format(args.name), '{:s}-{:s}-{:s}'.
                                      format(script_name_stem, args.ID, current_time_str)))
        else:
    ...
    if args.evaluate:
        ...
        train_dataset = get_instance(custom_datasets, '{0}_train'.format(args.dataset.name), args, **args.dataset.args)
        ...
        if i % args.print_freq == 0:
            print_func('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Prec@{kout} {top5.val:.3f} ({top5.avg:.3f})'.format(
                   ...
                   data_time=data_time, loss=losses, top1=top1, kout=kout, top5=top5))
