...
from TextClassification.model_DAN_2constraints import CNN_Embed_v2_withFeature as Text_Embed
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, text_model, criterion, optimizer, epoch, args, print_func)
...
def train(train_loader, model, text_model, criterion, optimizer, epoch, args, print_func):
    ...
    for i, (input, target, text) in enumerate(train_loader):
        ...
        x_weights = torch.sigmoid(torch.bmm(word_features, output_proj.unsqueeze(2)))
        # x_weighted_global = torch.sum((x_anchors.permute([0, 2, 1]) * x_weights, (1,), keepdim=False)

        x_weighted_global = torch.bmm(word_features.permute([0, 2, 1]),  x_weights).squeeze(-1)


        log_softmax_output = F.log_softmax(output_cls, dim=1)
        loss_cls = - torch.sum(log_softmax_output * target) / output_cls.shape[0]

        loss_ebd = torch.sum(1 - cos(output_proj, x_weighted_global)) / output_proj.shape[0]
