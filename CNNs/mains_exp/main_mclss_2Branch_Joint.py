...
# TODO: compared to 2Branch, this version maps image and text into the same space, on the same space it does
...
def main():
    ...
    text_generator = Text_Embed(**text_params)
    text_generator.load_state_dict(text_data['model'].state_dict(), strict=False)
    ...
    if args.gpu is not None:
        text_generator = text_generator.cuda(args.gpu)
        text_model = text_model.cuda(args.gpu)
    else:
        # text_generator = text_generator.cuda()
        # text_model = text_model.cuda()
        text_model = torch.nn.DataParallel(text_model).cuda()
        text_generator = torch.nn.DataParallel(text_generator).cuda()
    ...
    print_func("Total visual Parameters: {0}\t Gradient Parameters: {1}".format(model_total_params, model_grad_params))

    model_total_params = sum(p.numel() for p in text_model.parameters())
    model_grad_params = sum(p.numel() for p in text_model.parameters() if p.requires_grad)
    print_func("Total text Parameters: {0}\t Gradient Parameters: {1}".format(model_total_params, model_grad_params))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, visual_model, text_model, text_generator, criterion, optimizer, epoch, args, print_func)
...
def train(train_loader, visual_model, text_model, text_generator, criterion, optimizer, epoch, args, print_func):
    ...
    for i, (visual_input, visual_target, text) in enumerate(train_loader):
        ...
        loss_text_cls = - torch.sum(log_softmax_text * text_target) / text_cls.shape[0]


        log_softmax_visual = F.log_softmax(visual_cls, dim=1)
        loss_visual_cls = - torch.sum(log_softmax_visual * visual_target) / visual_cls.shape[0]

        loss_ebd = torch.sum(1 - cos(visual_proj, text_proj)) / visual_proj.shape[0]
        ...
        prec1 = CNN_utils.accuracy_multihots(visual_cls, visual_target, topk=(1, 3))
        ...
        if i % args.print_freq == 0:
            print_func('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  .format(
                   ...
                   data_time=data_time, losses_visual_cls=losses_visual_cls, losses_text_cls=losses_text_cls,
